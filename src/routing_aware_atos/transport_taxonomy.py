...
def classify_feature_transport(
    ...
    *,
    ...
    cross_r2 = max(
        [float(feature_scores.get(name, -1.0)) for name in cross_token_policies]
        or [-1.0]
    )
    multi_r2 = max(
        [float(feature_scores.get(name, -1.0)) for name in multi_source_policies]
        or [-1.0]
    )
...
def build_feature_policy_matrix(
    eval_payloads: Sequence[Mapping[str, Any]],
) -> dict[int, dict[str, float]]:
    """
    Expects payload rows with:
        payload["policy_name"]
        payload["feature_metrics"]
    """
...
def build_transport_taxonomy(
    ...
    *,
    ...
    feature_policy_matrix = build_feature_policy_matrix(eval_payloads)
