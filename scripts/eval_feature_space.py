...
def main() -> None:
    ...
    if cfg.get("pairs_path"):
        payload = evaluate_operator_in_feature_space(
            ...
            activated_only=cfg.get("activated_only", False),
            min_feature_activations=cfg.get("min_feature_activations", 1),
            compute_device=cfg.get("compute_device", "cpu"),
            metric_batch_size=cfg.get("metric_batch_size", 2048),
        )
    else:
        ...
        payload = evaluate_operator_from_cached_samples(
            ...
            output_path=output_path,
            split_name=cfg.get("split_name", "eval"),
            filter_split=cfg.get("filter_split", False),
            activated_only=cfg.get("activated_only", False),
            min_feature_activations=cfg.get("min_feature_activations", 1),
            compute_device=cfg.get("compute_device", "cpu"),
            metric_batch_size=cfg.get("metric_batch_size", 2048),
